	...
	function PLUGIN:PostPlayerLoadout(client)
		...
		timer.Create(uniqueID, 0.25, 0, function()
			...
			if(character:HasTrait("frozenbones") and client:KeyDown(IN_SPEED)) then
				...
				if(filter:GetCount() <= 3) then
					return
				end
				
				if(client:GetCharacter():GetData("LastCheckTimeAgi", 0) - RealTime() > 43200 ) then
					client:GetCharacter():SetData("LastCheckTimeAgi", 0)
				end

				if(client:GetCharacter():GetData("LastCheckTimeAgi", 0) == 0) then
					client:GetCharacter():SetData("LastCheckTimeAgi", RealTime() + 43200 )
				end
                    
				if(client:GetCharacter():GetData("LastCheckTimeAgi", 0) < RealTime()) then
					client:GetCharacter():SetData("LastAttributeAgiCheck", 0)
					client:GetCharacter():SetData("LastCheckTimeAgi", 0)
				end

				if(client:GetCharacter():GetData("LastAttributeAgiCheck", 0) == 0) then
					client:GetCharacter():SetData("LastAttributeAgiCheck", character:GetAttribute("agi"))
				end

				if(client:GetCharacter():GetData("LastAttributeAgiCheck", 0) + 0.5 > character:GetAttribute("agi")) then
		    		character:UpdateAttrib("agi", 0.0005)
				end
			end
			...
			if (current != value) then
				...
				if (value == 0 and !client:GetNetVar("brth", false)) then
					...
					if(client:GetCharacter():GetData("LastAttributeAgiCheck", 0) + 0.5 > character:GetAttribute("agi")) then
		    			character:UpdateAttrib("agi", 0.001)
    				end
...
--					character:UpdateAttrib("agi", 0.01)
	...
	function PLUGIN:PlayerHurt(client , attacker)
		...
		if(client:GetCharacter():GetData("LastCheckTime", 0) - RealTime() > 43200 ) then
			client:GetCharacter():SetData("LastCheckTime", 0)
		end

		if(client:GetCharacter():GetData("LastCheckTime", 0) == 0) then
			client:GetCharacter():SetData("LastCheckTime", RealTime() + 43200 )
		end

		if(client:GetCharacter():GetData("LastCheckTime", 0) < RealTime()) then
			client:GetCharacter():SetData("LastAttributeConCheck", 0)
			client:GetCharacter():SetData("LastCheckTime", 0)

		end

		if(client:GetCharacter():GetData("LastAttributeConCheck", 0) == 0) then
			client:GetCharacter():SetData("LastAttributeConCheck", character:GetAttribute("con"))
		end

		if(client:GetCharacter():GetData("LastAttributeConCheck", 0) + 0.5 > character:GetAttribute("con")) then
