...
function PLUGIN:CharacterLoaded(character)
	PLUGIN:SetCustomHeight(character:GetPlayer())
...
--		["9'0"] = 1.40
	...
	if (heightTable[foundHeight]) then
		...
		client:SetModelScale(scale, 1)
		for k,v in pairs(boneTable) do
			local boneID = client:LookupBone(v)
			local boneScale = 1 / scale
			client:ManipulateBoneScale(boneID, Vector(boneScale, boneScale, boneScale))
		end
	end
end
--[[
