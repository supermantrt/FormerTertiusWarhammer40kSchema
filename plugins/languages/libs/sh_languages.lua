...
LANGUAGE_TABLE.color = Color(34, 85, 85, 255)
...
function ix.languages:Register(language)
	...
	-- Yelling
	ix.command.Add('yell' .. string.Replace(language.uniqueID, 'language_', ''), {
		arguments = ix.type.text,
		...
		OnRun = function(this, client, message)
			...
			if !message or string.Trim(message) == "" then return end
			if client:GetCharacter():CanSpeakLanguage(language.uniqueID) then
				ix.chat.Send(client, "yell" .. language.uniqueID, message)
				ix.chat.Send(client, "yell" .. language.uniqueID .. "_unknown", message)
			else
				client:Notify('You don\'t know such a language!')
			end
		end
	...
	ix.chat.Register("yell" .. language.uniqueID, {
	...
	ix.chat.Register("yell" .. language.uniqueID .. "_unknown", {
	...
	if (CLIENT) then
		ix.command.list["yell"..string.Replace(language.uniqueID, 'language_', '')].OnCheckAccess = function(self, client) return client:GetCharacter():CanSpeakLanguage(language.uniqueID) end
		...
		CHAT_RECOGNIZED["yell" .. language.uniqueID .. "_unknown"] = true
		CHAT_RECOGNIZED["yell" .. language.uniqueID] = true
