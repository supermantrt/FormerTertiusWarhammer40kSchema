...
function PANEL:Init()
	...
	self.backgroundScrollPanel = self.backgroundPanel:Add("DScrollPanel")
	self.backgroundScrollPanel:Dock(FILL)
...
function PANEL:GetContainerPanel(name)
	...
	if ( self.containerPanels[ name ] ) then
		return self.containerPanels[ name ]
	end
...
function PANEL:Populate()
	...
	if (!self.bInitialPopulate) then
		...
		if (#self.backgroundPanel:GetChildren() > 1) then
			self.progress:AddSegment("@background")
		end
