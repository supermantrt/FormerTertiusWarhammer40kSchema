...
function ITEM:GetDescription()
	if (self.entity) then
		return L(self.description .. "\n \n Durability:" .. math.floor(self:GetData("durabilityvar", self.Durability)))
	else
...
function ITEM:OnInstanced(invID)
	...
	self:SetData("durabilityvar", self.Durability)

	local ply = inventory:GetOwner()

	if(ply:GetNetVar("ArmourFlip", false) == true) then
		self:SetData("durabilityvar", ply:GetNetVar("OldArmourValue", self.Durability))
		ply:SetNetVar("ArmourFlip", false)
	end
end
...
ITEM.functions.repair = {
	...
	OnRun = function(item)
		...
		local number = 0
		if(client:Team() == FACTION_MEC) then
		    if (character:GetAttribute("int") >= 5) then
			    for k, v in pairs(items) do
			    	if (v:GetName() == item.RepairItem and number < 2) then
			    		for i=0, 1 do
			    			number = number + 1
			    			if ( v:GetData('stacks', 1) > 1 ) then
								v:SetData('stacks', v:GetData('stacks', 1) - 1)
							else
								v:Remove()
								break
							end
		    	...
		    	if (number == 2) then
			    	item:SetData("durabilityvar", item.Durability)
			    	number = 0
		        else
			        client:Notify("You need two materials to do this")
			    end
		    else
		    	client:Notify("You need a better level of intelligence to repair this")
		    end
		end
		if(client:Team() ~= FACTION_MEC and client:GetCharacter():GetData("CanRepair", false) == true) then
		    if (character:GetAttribute("int") >= 10) then
			    for k, v in pairs(items) do
			    	if (v:GetName() == item.RepairItem and number < 5) then
				    	for i=0, 3 do
				    		number = number + 1
			    			if ( v:GetData('stacks', 1) > 1 ) then
								v:SetData('stacks', v:GetData('stacks', 1) - 1)
		    	...
		    	if (number == 4) then
			    	item:SetData("durabilityvar", (item.Durability * 0.75))
			    	number = 0
		        else
			        client:Notify("You need four materials to do this")
			    end
		    else
		    	client:Notify("You need a better level of intelligence to repair this")
		    end
...
ITEM.functions.Equip = {
	...
	icon = "icon16/tick.png",
	OnRun = function(item)
		if (item:GetData("durabilityvar" , item.Durability) >= (item:GetData("durabilityvar" , item.Durability) * 0.5)) then
			local client = item.player
...
function ITEM:CanEquipOutfit()
	if (self:GetData("durabilityvar", self.Durability) <= (self:GetData("durabilityvar", self.Durability) * 0.5)) then
		return false
	else
		return true
	end
